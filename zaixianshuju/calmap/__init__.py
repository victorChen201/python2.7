...
def calendarpollutants(data, pollutants, year=None, how='sum',  yearlabels=True, yearascending=True, yearlabel_kws=None,
                 subplot_kws=None, gridspec_kws=None, fig_kws=None, **kwargs):
    """
    Plot a timeseries as a calendar heatmap.
    ...
    fig, axes = plt.subplots(nrows=len(pollutants), ncols=1, squeeze=False,
                             subplot_kw=subplot_kws,
                             gridspec_kw=gridspec_kws, **fig_kws)
    axes = axes.T[0]
    ...
    for pollutant, ax in zip(pollutants, axes):
        by_day = data[pollutant].resample('D').mean()
        yearplot(by_day, year=year, how='Mean', ax=ax, **kwargs)
        max_weeks = max(max_weeks, ax.get_xlim()[1])

        if yearlabels:
            ax.set_ylabel(pollutant, **ylabel_kws)
