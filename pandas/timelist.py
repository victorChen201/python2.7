...
df = pd.read_csv("t2013.csv", index_col = 'TimePoint',  parse_dates= True)
df = df.select_dtypes(include= ['float64'])

meanValues = pd.DataFrame(columns=collist)
meanValues.loc[0] = df.loc[sprdeadline[0]:sprdeadline[1],collist].mean()
meanValues.loc[1] = df.loc[sumdeadline[0]:sumdeadline[1],collist].mean()
meanValues.loc[2] = df.loc[autdeadline[0]:autdeadline[1],collist].mean()
meanValues.loc[3] = df.loc[windeadline[0]:windeadline[1],collist].append(df.loc[windeadline[2],collist]).mean()
meanValues.index = legend
ax = meanValues.plot(kind='bar',rot=360)
ax.set_ylabel(u'质量浓度(u/m3)')

#plt.style.use('classic')这个会导致中文输出方框
'''
