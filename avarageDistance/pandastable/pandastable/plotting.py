...
class PlotViewer(Frame):
    ...
    def loadMap(ax, projection='cyl', llcrnrlon=0.0, llcrnrlat=0.0, urcrnrlon=357.5, urcrnrlat=90.0):
    
        start = time.time()
        m = Basemap(llcrnrlon=llcrnrlon, llcrnrlat=llcrnrlat, urcrnrlon=urcrnrlon, urcrnrlat=urcrnrlat, projection=projection, ax=ax)
        m.readshapefile('CHN_adm1', 'China', color='gray')
        return m

    def setBar(ax, data, width=0.155, factor=0.03, colors = ['r', 'g', 'b', 'y', 'c'], edgecolor="none", align="center"):
       
        bars = []
        df = data[['SO4','NO3','NH4','OC','EC']]
        # for i, attr in enumerate(data.columns[3:]):
        for i, attr in enumerate(df.columns[:-1]):
            left = [j + width * i for j in data['Latitude']]
            height = [j * factor for j in data[attr]]
            aBar = ax.bar(left, height, width=width, bottom=data['Longtitude'], color=colors[i], align=align, edgecolor=edgecolor)
            bars.append(aBar)
        height = [j * factor for j in df['EC']]
        heightOC = list(map(lambda x: x[0] + x[1], zip(data['Longtitude'], [rect.get_height() for rect in bars[-1]])))
        aBar = ax.bar(left, height, width=width, bottom=heightOC, color=colors[-1], align=align, edgecolor=edgecolor)
        bars.append(aBar)
        ax.legend(bars, df)
        ax.set_xticklabels(data['site'])
        return bars
    ...
    def _doplot(self, data, ax, kind, subplots, errorbars, useindex, bw, kwargs):
        ...
        if kind == 'distribution':
            # m = self.loadMap(ax, projection='cyl', llcrnrlon=0.0, llcrnrlat=0.0, urcrnrlon=357.5, urcrnrlat=90.0)  
            bars = []
            width=0.2
            factor=0.03
            colors = ['r', 'g', 'b', 'y', 'c']
            ...
            for i, attr in enumerate(df.columns[:-1]):
                left = [j + width * i for j in data['Latitude']]
                height = [j * factor for j in data[attr]]
                aBar = ax.bar(left, height, width=width, bottom=data['Longtitude'], color=colors[i], edgecolor="none")
                bars.append(aBar)
            # height = [j * factor for j in df['EC']]
            # heightOC = list(map(lambda x: x[0] + x[1], zip(data['Longtitude'], [rect.get_height() for rect in bars[-1]])))
            # aBar = ax.bar(left, height, width=width, bottom=heightOC, color=colors[-1],  edgecolor="none")
            # bars.append(aBar)
            # ax.legend(bars, df)
            # ax.set_xticklabels(data['site'])
            # bars = self.setBar(ax, data, width=0.2, factor=0.03, colors = ['r', 'g', 'b', 'y', 'c'], edgecolor="none")
        elif kind == 'boxplot':
...
class MPLBaseOptions(TkOptions):
    ...
    linestyles = ['-','--','-.',':','steps']
    kinds = ['line', 'scatter', 'bar', 'distribution','barh', 'pie', 'histogram', 'boxplot',
